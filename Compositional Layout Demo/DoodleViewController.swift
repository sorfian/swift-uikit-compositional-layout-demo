...
class DoodleViewController: UIViewController {
    ...
    private var officeImages = (1...15).map { index in
        "office-\(index)"
    }
    
    private var kitchenImages = (1...25).map { index in
        "kitchen-\(index)"
    }
    
    private var macbookImages = (1...30).map { index in
        "macbook-\(index)"
    }
...
//    }
    ...
    private func createMultiGridLayout() -> UICollectionViewLayout {
        let layout = UICollectionViewCompositionalLayout { (sectionIndex, layoutEnvironment) -> NSCollectionLayoutSection? in
            ...
            let groupSize = NSCollectionLayoutSize(widthDimension: .fractionalWidth(1.0 ),heightDimension: .absolute(sectionIndex == 0 ? 200.0 : 80.0))
            let group = NSCollectionLayoutGroup.horizontal(layoutSize: groupSize, subitems: [item])
